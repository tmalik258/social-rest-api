...
class TestPostViewSet:
    ...
    def test_create(self, client, user, post):
        ...
        data = {"author": user.public_id.hex, "body": "Test Post Body"}
    ...
    def test_update(self, client, user, post):
        ...
        data = {"author": user.public_id.hex, "body": "Test Post Body (updated)"}
    ...
    def test_update_anonymous(self, client, post):
        data = {"author": "test_user", "body": "Test body post (updated)"}
    ...
    def test_like(self, client, user, post):
        ...
        response = client.post(f"{self.endpoint}{post.public_id}/toggle_like/")

        assert response.status_code == status.HTTP_200_OK
        assert response.data["action"] == "liked"

    def test_unlike(self, client, user, post):
        client.force_authenticate(user=user)

        client.post(f"{self.endpoint}{post.public_id}/toggle_like/") # like

        response = client.post(f"{self.endpoint}{post.public_id}/toggle_like/") # unlike
